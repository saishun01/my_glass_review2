...
delta = 1
#n = 1.11 #+ delta  # n=10でないと生きられないネットも
...
### lagを見るための栄養濃度
#n1 = 0.99999 # これでギリギリ耐える（err=0, 時間10^5）
n1 = 0
n2 = 2
###
...
#fig.suptitle(r'Reaction = {}, $\,n = {} \to {}$'.format(Rlist, n1, n2))
...
t,x = calc(Rlist, n1, s, x0)
print(x[-1,:])


t,x = calc(Rlist, n2, s, x[-1,:])

for i in range(len(x[0,:])):
    ax.plot(t,x[:,i], label=r'$x_{}$'.format(i))
    #if i == 1:
    #    ax.plot(t,np.ones_like(x[:,1])-x[:,1], label='{}'.format(i))
ax.set_title(r'Reaction = {}, $\,n = {} \to {}$'.format(Rlist, n1, n2))
ax.set_xlabel('Time')
...
for s in range(2):
    ...
    for i in range(len(x[0,:])):
        ax[s].plot(t,x[:,i], label=r'$x_{}$'.format(i))
        #if i == 1:
        #    ax.plot(t,np.ones_like(x[:,1])-x[:,1], label='{}'.format(i))
    ax[s].set_title(r'Error rate depending on $x_{}$'.format(s))
    ax[s].set_xlabel('Time')
    ax[s].set_ylabel('Concentration')

    #ax[s].set_xscale('log')
    #ax[s].set_xlim(0,10**5)
    #ax[s].set_xlim(0,10)

    ax[s].set_yscale('log')
    #ax[s].set_ylim(10**(-10),10**4)
    #ax[s].set_ylim(10**(-12),10**4)
    #ax[s].set_ylim(0,1)
