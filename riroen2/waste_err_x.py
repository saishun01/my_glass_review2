...
#list_err = [0.001, 0.01, 0.1, 0.5]
list_err = [0.001, 0.01, 0.1, 1]
list_n = [10**(-4), 10**(-3), 10**(-2), 10**(-1), 10**0, 10**1, 10**2, 10**3, 10**4]
#list_n = [1,10]

Nseed = 5

#color_cycle= itertools.cycle(['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf'])
#color_cycle= itertools.cycle(["blue","orange"])
fig = plt.figure()
...
for j in range(len(list_err)):
    ...
    for i in range(len(list_n)):
        ...
        for s in range(Nseed):
            ...
            # 成長率
            list_x.append(ni)
            list_y.append(x[-1,N]) # 膜分子のゴミを見る

    ax.scatter(list_x, list_y,color=cmap(j), label='error rate = {}'.format(list_err[j]))

ax.set_xlabel(r'$n$')
ax.set_ylabel('Waste of membrane')
ax.set_xscale('log')
ax.set_yscale('log')
